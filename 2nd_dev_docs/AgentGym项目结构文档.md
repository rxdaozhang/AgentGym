...
- **BaseEnvClient**: 环境客户端抽象基类，定义了环境客户端的接口
  ```python
...
- **Evaluator**: 评估器类，继承自BaseAgentEnvController
  ```python
...
4. **数据格式约定**:
   - 使用ReAct格式组织对话和行动
   - 交互记录使用`ConversationMessage`格式存储
   - 经验数据使用`ExperienceOutput`格式存储
   - 评估结果使用`EvaluationOutput`格式存储